...
def inputinterpret(line, pins, sensorsdict):
    if "/pins" in line:
        print("PINSPINSPINSPINSPINS")
        for i in pins:
            if str(i) in line:
                pin = {str(i): i.value()}
                pindumped = json.dumps(pin)
                return str(pindumped)
        pinscopy = pins
        pinsdict = {}
        for i in pinscopy:
            pinsdict[str(i)] = i.value()
        pinsdumped = json.dumps(pinsdict)
        print("PINSDUMPED" + str(pinsdumped))
        return str(pinsdumped)
    if "/sensors" in line:
        print("SENSORSSENSORSSENSORSSENSORS")
        sensorcopy = sensorsdict
        sensdict = {}
        for i in sensorsdict:
            if str(i) in line:
                if str(i) == "Temperature":
                    sensdict[str(i)] = temp_c(data)
                    sensdumped = json.dumps(sensdict)
                    return str(sensdumped)
                elif str(i) == "Testsensor":
                    sensdict[str(i)] = 69
                    sensdumped = json.dumps(sensdict)
                    return str(sensdumped)
        for sensor in sensorcopy:
            if str(sensor) == "Temperature":
                sensdict[str(sensor)] = temp_c(data)
            elif str(sensor) == "Testsensor":
                sensdict[str(sensor)] = 69
        sensdumped = json.dumps(sensdict)
        return str(sensdumped)
